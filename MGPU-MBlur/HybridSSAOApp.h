...
class HybridMBlurApp final :
    public Common::D3DApp
{
public:
    HybridMBlurApp(HINSTANCE hInstance);
    ~HybridMBlurApp() override;
...
protected:
    ...
    void PopulateMbTexturesCommands(const std::shared_ptr<GCommandList>& cmdList) const;
    void PopulateMbCommands(const std::shared_ptr<GCommandList>& cmdList) const;
    ...
    // MB part
    void UpdateMbCB(const GameTimer& gt) const;
    Matrix matrixViewPrev = Matrix::Identity;
    ...
    bool IsUsingSharedMB = false;//true;
